...
export default function QuickAddTransactionDrawer() {
  const { quickAddOpen, setQuickAddOpen, selectedMonth, defaultCurrency } =
    useAppStore();
  ...
  const accounts = (accountsData?.accounts ?? []).filter((a) => !a.off_budget);
  ...
  useEffect(() => {
    if (quickAddOpen && accounts.length > 0) {
      // Find first active liquid account or any account
      const defaultAcc =
        accounts.find(
  ...
  }, [quickAddOpen, accounts, toAccountId, accountId]);
  ...
  // 1. Calculate top 5 payees and predictive envelopes based on transaction history
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col justify-end sm:items-center sm:justify-center md:inset-auto md:bottom-20 md:right-6 md:w-[500px] md:h-auto md:max-h-[85vh]">
      ...
      <div
        className={cn(
          "relative w-full max-w-lg md:max-w-[500px] bg-card border border-border shadow-2xl rounded-t-2xl sm:rounded-2xl flex flex-col overflow-hidden max-h-[92dvh] md:max-h-[83vh] transition-all duration-300 ease-out transform translate-y-0 md:translate-y-0",
          successAnimation &&
            "scale-[0.98] border-positive/50 bg-positive/5 dark:bg-positive/20"
        )}
      >
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar px-5 py-4 space-y-4">
          {/* Amount input block (Double tall, heavy numbers) */}
                  ...
                  .map((a) => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "flex-shrink-0 w-36 rounded-xl border p-3 flex flex-col justify-between text-left transition-all duration-200 shadow-sm relative overflow-hidden select-none",
                          isSelected
                            ? "border-primary/50 bg-gradient-to-br from-primary/10 to-primary/5 ring-1 ring-primary/50 shadow-primary/5"
                            : "border-border/15 bg-background hover:bg-muted/20 hover:border-muted-foreground/10 shadow-sm"
                        )}
                      >
                        {isSelected && (
                          <div className="absolute top-2 right-2 w-3.5 h-3.5 rounded-full bg-primary flex items-center justify-center border border-background">
        ...
        {/* Sticky Bottom Actions: Houses the gorgeous Submit log button and native safe areas */}
        <div className="border-t border-border/10 bg-muted/20 p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] flex-shrink-0">
          <button
